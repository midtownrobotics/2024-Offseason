  ...
  public void updateState() {
    ...
    switch (currentState) {
      ...
      case AUTO_AIM_REVSHOOT:
        shooter.setState(ShooterState.AUTO_AIM_REVVING);
        intake.setState(IntakeState.IDLE);
        if (shooter.getFlywheelSpeed()
                >= (shooter.getSpeedFromDistance() - ShooterConstants.SPEAKER_SPEED_TOLERANCE.get())
            && (shooter.getPivotAngle()
                    >= (shooter.getAngleFromDistance()
                        - ShooterConstants.PIVOT_ANGLE_TOLERANCE.get())
                && shooter.getPivotAngle()
                    <= (shooter.getAngleFromDistance()
                        + ShooterConstants.PIVOT_ANGLE_TOLERANCE.get()))) {
